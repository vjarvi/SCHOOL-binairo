...
{
    std::cout << "x: " << x << "y: " << y << "char: " << symbol_char;
...
std::vector<std::vector<std::string>> GameBoard::return_board()
{
    // basically copies board_ to map that has 0, 1 or empty as a string
    std::vector<std::vector<std::string>> returned_map;

    int i = 0;
    int j = 0;
    for ( const auto &row : board_ ){
        returned_map.push_back(std::vector<std::string>());

        for ( const auto &value : row){

            if ( value == 0 ){
                returned_map[i].push_back("0");

            }else if ( value == 1 ){
                returned_map[i].push_back("1");

            } else if ( value == 2 ){
                returned_map[i].push_back(" ");
            }
            j++;
        }
        i++;
